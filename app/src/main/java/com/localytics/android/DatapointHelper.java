...
/* package */final class DatapointHelper
{
    ...
    /**
     * Gets the device's Advertising ID if user's app is using play service.
     *
     * @param context The context used to access the play service.
     * @return The Advertising ID. May return null if Advertising ID is not available.
     */
    public static String getAdvertisingIdOrNull(final Context context)
    {
        // If user's app is not using play service, return null.
        String advertisingId = null;

        try
        {
            // Get AdvertisingIdClient.Info
            Object adInfo = ReflectionUtils.tryInvokeStatic("com.google.android.gms.ads.identifier.AdvertisingIdClient", "getAdvertisingIdInfo", new Class<?>[] { Context.class }, new Object[] { context });

            if (null != adInfo)
            {
                advertisingId = ReflectionUtils.tryInvokeInstance(adInfo, "getId", null, null);
            }
        }
        catch (final Exception e)
        {
            if (Constants.IS_LOGGABLE)
            {
                Log.w(Constants.LOG_TAG, "Localytics library can't get the advertising id", e); //$NON-NLS-1$
            }
        }
